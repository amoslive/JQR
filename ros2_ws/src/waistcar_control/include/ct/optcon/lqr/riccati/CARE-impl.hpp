                                         ...
                                         solver_t solver)
{
    ...
    switch (solver) {
        ...
        case DIRECT_LAPACK:
            return solveSchurDirect(M, P);
        case DIRECT_MANUAL:
            return solveRiccatiArimotoPotter(M, P);
        default:
            return solveSchurIterative(M, P);
    }
}
...
template <size_t STATE_DIM, size_t CONTROL_DIM>
bool CARE<STATE_DIM, CONTROL_DIM>::solveRiccatiArimotoPotter(const schur_matrix_t& Ham, state_matrix_t &P) {
    ...
    Eigen::MatrixXcd eigvec = Eigen::MatrixXcd::Zero(2 * STATE_DIM, STATE_DIM);

    int j = 0;
    for (int i = 0; i < 2 * STATE_DIM; ++i) {
        if (Eigs.eigenvalues()[i].real() < 0.) {
            eigvec.col(j) = Eigs.eigenvectors().block(0, i, 2 * STATE_DIM, 1);
            ++j;
        }
    }
    ...
    Vs_1 = eigvec.block(0, 0, STATE_DIM, STATE_DIM);
    Vs_2 = eigvec.block(STATE_DIM, 0, STATE_DIM, STATE_DIM);
